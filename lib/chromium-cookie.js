// stolen from https://github.com/bertrandom/chrome-cookies-secure
// modified by @Sunesu - added support for more chromium based browsers
...
/**
 * Gets browser user data directory
 */
function getBrowserCookiesPath(browser,profile = 'Default'){
    switch (browser) {
        case 'chrome':
            ...
            break;
        case 'brave':
            path = `${process.env.LOCALAPPDATA}\\BraveSoftware\\Brave-Browser\\User Data\\${profile}\\Network\\Cookies`;
            break
        case 'operagx':
            path = `${process.env.APPDATA}\\Opera Software\\Opera GX Stable\\Network\\Cookies`;
            break;
        default:
            break;
    }
...
function getBrowserLocalStatePath(browser){
    switch (browser) {
        case 'chrome':
            path = os.homedir() + '/AppData/Local/Google/Chrome/User Data/Local State'
            break;
        case 'brave':
            path = `${process.env.LOCALAPPDATA}\\BraveSoftware\\Brave-Browser\\User Data\\Local State`
            break;
        case 'operagx':
            path = `${process.env.APPDATA}\\Opera Software\\Opera GX Stable\\Local State`
            break;
        default:
            break;
    }
...
const getCookies = async (uri, format, callback, browser='chrome', profile='Default') => {
    ...
    if (process.platform === 'darwin') {
        ...
        path = getBrowserCookiesPath(browser,profile);
				...
				if (cookie.value === '' && cookie.encrypted_value.length > 0) {
					...
					if (process.platform === 'win32') {
						if (encryptedValue[0] == 0x01 && encryptedValue[1] == 0x00 && encryptedValue[2] == 0x00 && encryptedValue[3] == 0x00){
							...
							localState = JSON.parse(fs.readFileSync(getBrowserLocalStatePath(browser)));
 ...
 */
const getCookiesPromised = async (uri, format, browser, profile) => {
